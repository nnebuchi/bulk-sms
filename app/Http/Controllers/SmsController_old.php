...
class SmsController extends Controller
{
    ...
    public function sent(){
        $data['messages'] = Message::where('user_id', Auth::user()->id)->where('status', '>=', '1')->where('status', '<=', '3')->with('contacts')->with('messageStatus')->latest()->get();
    ...
    public function draft(){
        $data['messages'] = Message::where('user_id', Auth::user()->id)->where('status', '=', '0')->latest()->get();
